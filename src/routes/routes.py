...
def get_huggingface_username(token: str) -> str:
    """
    Retrieve the Hugging Face username using the token. If not logged in, login first using the token.
    
    Args:
        token (str): The Hugging Face API token.
    
    Returns:
        str: The username or 'unknown' if retrieval fails.
    """
    try:
        # Step 1: Log in using the provided token
        ...
        if login_process.returncode != 0:
            logger.error(f"Failed to login with Hugging Face token: {token}")
            return "unknown"

        # Step 2: Now run the whoami command
        result = subprocess.run(
            ["huggingface-cli", "whoami"],
            check=True, capture_output=True, text=True
        )

        # Step 3: Parse the result and extract the username
        user_info = json.loads(result.stdout)
        return user_info.get("name", "unknown")

    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to get Hugging Face username: {str(e)}")
        return "unknown"
...
async def finetune_model(
    ...
    try:
        ...
        if hf_dataset_repo:
            ...
            hugginface_name = get_huggingface_username(token)
            repo_id = f"{hugginface_name}/dataset_{model_type}_{language}_{voice_name}"
